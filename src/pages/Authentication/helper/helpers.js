...
export async function getUserByEmail(email) {
  try {
    const response = await axios.get(`http://localhost:5000/api/userByEmail/${email}`);
...
export async function generateOTP(emailParam) {
  try {
    const response = await axios.get('http://localhost:5000/api/generateOTP', { params: { email: emailParam } });
